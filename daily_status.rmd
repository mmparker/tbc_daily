...
# Query the RVCT
# TODO: Add a check for disease site
rvct <- sqlQuery(plus, "
                 
    SELECT r.person_id, p.last_name, p.first_name, p.date_of_birth,
           r.therapystart, r.conversion, r.conversiondate
    ...
    WHERE r.therapystart >= (Date() - 90)
        ...
        AND r.verification = 1
    ORDER BY r.therapystart
                 
")

# Add a 60-days-after-tx-start indicator
rvct$sixtydays <- as.Date(rvct$therapystart) + 60

# Recode conversion status
rvct$conversion[rvct$conversion %in% c(0, 2, 9)] <- "No"
rvct$conversion[rvct$conversion %in% 1] <- "Yes"

# Pretty names
names(rvct) <- c("TBdb ID", "Last Name", "First Name", "DOB", "Tx Start", 
                 "Documented Sputum Conversion?", "Conversion Date", "60 Days After Tx Start")

dfprintif(rvct[ , c("TBdb ID", "Last Name", "First Name", "DOB", 
                    "Tx Start", "60 Days After Tx Start",
                    "Documented Sputum Conversion?")],
