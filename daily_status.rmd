...
weeklytx <- ddply(weeks, .var = "monday", .fun = function(x) {
    ...
    # Tag plans that started before or during the week, and those that ended during or after
    xplans$start_bod <- with(xplans, as.Date(treat_plan_date) <= x$friday)
    xplans$end_doa <- with(xplans, as.Date(treat_plan_end) >= x$monday)
...
weeklytx.m$variable[weeklytx.m$variable %in% "n_active"] <- "Active Therapy"
weeklytx.m$variable[weeklytx.m$variable %in% "n_ltbi"] <- "LTBI Therapy"
